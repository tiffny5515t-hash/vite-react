...
// --- Firebase 配置 ---
// 自動判斷環境：優先使用系統注入的配置，若無則使用手動填寫的配置
...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'merchant-risk-backend-v1';
    ...
    <div className="min-h-screen bg-[#0a0a0c] text-slate-300 font-sans p-4 md:p-8">
      ...
      <header className="max-w-[1600px] mx-auto mb-10 flex flex-col lg:flex-row justify-between items-center gap-6 border-b border-slate-800 pb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-white uppercase tracking-tighter">Merchant Risk System</h1>
            <p className="text-[10px] text-slate-500 uppercase tracking-[0.4em] font-bold mt-1">商戶聯動風控管理後台</p>
          </div>
        </div>
        <nav className="flex bg-slate-900/60 p-1.5 rounded-2xl border border-slate-800">
          ...
          <button onClick={() => setActiveTab('engine')} className={`px-8 py-3 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${activeTab === 'engine' ? 'bg-emerald-600 text-black shadow-lg' : 'text-slate-500 hover:text-white'}`}><Repeat size={16} /> 聯動掃描引擎</button>
        </nav>
      </header>
        ...
        <div className="lg:col-span-4 space-y-6">
          {activeTab === 'manager' ? (
            <section className="bg-slate-900/40 border border-slate-800 rounded-3xl p-6 backdrop-blur-md sticky top-28">
              ...
              {addMode === 'single' ? (
                <div className="space-y-4">
                  ...
                  <button onClick={addMerchant} className="w-full bg-emerald-600 hover:bg-emerald-500 text-black font-black py-4 rounded-xl transition-all uppercase text-xs">儲存至雲端</button>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  <button onClick={addBulkMerchants} className="w-full bg-emerald-600 hover:bg-emerald-500 text-black font-black py-4 rounded-xl transition-all uppercase text-xs">執行批量匯入</button>
                </div>
              )}
              <div className="mt-8 border-t border-slate-800 pt-6 max-h-[400px] overflow-y-auto custom-scrollbar">
                <h3 className="text-[10px] font-bold text-slate-500 uppercase mb-4 tracking-widest">目前監控中 ({merchantWallets.length})</h3>
            ...
            <section className="bg-slate-900/40 border border-slate-800 rounded-3xl p-8 backdrop-blur-md sticky top-28 shadow-2xl">
              <h2 className="text-white font-bold mb-8 flex items-center gap-2 text-red-500 uppercase italic tracking-widest"><ShieldAlert size={20} /> 發起風險審計</h2>
                ...
                <div className="mt-10 space-y-4">
                  {[1,2,3,4].map(s => (
                    <div key={s} className={`flex items-center gap-4 text-[11px] transition-all ${analysisStep >= s ? 'opacity-100' : 'opacity-20'}`}>
                      ...
                      <span className="text-slate-400 font-bold uppercase tracking-widest italic">Process {s}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          {finalReport ? (
            <div className="bg-slate-900/60 border border-slate-800 rounded-[2.5rem] overflow-hidden shadow-2xl animate-in fade-in pb-10">
              <div className="p-10 bg-gradient-to-br from-[#0c0c0e] to-black border-b border-slate-800 flex justify-between items-start">
                <div>
                  <h3 className="text-3xl font-black text-white italic tracking-tighter uppercase mb-3">AI 風控審計報告</h3>
                  <div className="flex items-center gap-2 mt-4 bg-emerald-500/5 p-3 rounded-xl border border-emerald-500/10 overflow-hidden">
                    <p className="text-[12px] text-emerald-500 font-mono font-bold whitespace-nowrap overflow-x-auto select-all custom-scrollbar-h flex-1 py-1">TARGET_Q: {finalReport.qAddress}</p>
              ...
              <div className="p-10 space-y-12">
                {/* 1. Q 流水帳 */}
                <section>
                  <h4 className="text-[11px] font-bold text-slate-500 uppercase tracking-[0.3em] mb-6 flex items-center gap-2"><Clock size={16} className="text-emerald-500" /> 第一層：客戶 Q 最近流水紀錄</h4>
                ...
                <section>
                  <div className="flex items-center justify-between mb-8 px-2 tracking-widest">
                    <h4 className="text-[11px] font-bold text-red-500 uppercase flex items-center gap-2"><AlertTriangle size={16} /> 碰撞命中分析</h4>
                      ...
                      <div key={i} className="bg-slate-900/80 border border-slate-800 rounded-[2.5rem] p-10 relative overflow-hidden group hover:border-red-500/30 transition-all shadow-xl">
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center relative z-10">
                          <div className="md:col-span-3 border-r border-slate-800/50 pr-6">
                            ...
                            <div className="text-white font-black text-2xl mb-4 tracking-tighter uppercase">{m.store}</div>
                          ...
                          <div className="md:col-span-9 space-y-6 overflow-hidden">
                             ...
                             <div className="flex justify-end gap-3 mt-4">
                                ...
                                <button onClick={() => openDetailModal(m)} className="flex items-center gap-2 text-[10px] font-black text-black bg-emerald-500 hover:bg-emerald-400 px-8 py-3 rounded-xl transition-all shadow-lg uppercase tracking-widest"><Activity size={14}/> 查看證據</button>
                             </div>
                          </div>
                        </div>
                      </div>
                ...
                <div className="pt-10 border-t border-slate-800 flex justify-center">
                  <button onClick={resetAnalysis} className="flex items-center gap-3 text-xs font-bold text-slate-500 hover:text-white transition-all uppercase bg-slate-900/50 px-12 py-4 rounded-full border border-slate-800 hover:border-emerald-500/50 shadow-xl"><Repeat size={16} /> 啟動全新比對任務</button>
                </div>
              </div>
            </div>
          ) : (
            <div className="h-full min-h-[750px] flex flex-col items-center justify-center bg-slate-900/20 rounded-[4rem] border-2 border-dashed border-slate-800/50 text-slate-600 p-20 text-center">
               ...
               <h3 className="text-2xl font-black text-slate-400 tracking-tighter uppercase italic">Awaiting Audit</h3>
               <p className="text-sm mt-4 text-slate-500 max-w-sm italic leading-relaxed">請在左側輸入舉報地址。AI 將自動掃描所有監控店鋪，識別潛在的資金共控群體。</p>
            </div>
          )}
        </div>
      </main>
            ...
            <div className="p-8 border-b border-slate-800 flex justify-between items-center bg-gradient-to-r from-emerald-950/20 to-transparent px-12">
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-emerald-500 font-bold uppercase tracking-widest mt-1">Store: {modalData.store} · Verification Success</p>
                </div>
              </div>
            ...
            <div className="p-12 overflow-y-auto custom-scrollbar space-y-12">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-red-500/5 p-8 rounded-[2rem] border border-red-500/20 shadow-xl overflow-hidden">
                  <h5 className="text-[10px] font-bold text-red-400 uppercase mb-4 flex items-center gap-2 tracking-widest"><Link2 size={14}/> 碰撞命中點 (Target)</h5>
                  <div className="flex items-center justify-between gap-6 overflow-hidden">
                    ...
                    <tbody className="divide-y divide-slate-800/50">
                      <tr className="bg-emerald-500/10">
                        ...
                        <td className="px-6 py-8 text-center">
                          <div className="flex flex-col items-center gap-2"><ArrowRightLeft size={20} className="text-emerald-500 animate-pulse" /><span className="text-[9px] font-black bg-emerald-500 text-black px-2 py-0.5 rounded shadow-lg uppercase tracking-widest">Matched</span></div>
                        </td>
              ...
              <div className="bg-emerald-500/5 p-12 rounded-[3rem] border border-emerald-500/20 shadow-inner px-16 relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-4 mb-5"><CheckCircle2 size={32} className="text-emerald-500" /><h5 className="text-xl font-black text-white italic uppercase tracking-tighter underline decoration-emerald-500/30 underline-offset-4">AI 審計診斷結論</h5></div>
                  <p className="text-[16px] text-slate-400 leading-loose font-medium max-w-5xl italic">{modalData.description} 基於鏈上數據時間軸與金額的高匹配度，判定舉報者與該店客戶為資金共同控制實體。建議採取風控阻斷措施。</p>
                </div>
              </div>
            </div>
            <div className="p-10 border-t border-slate-800 bg-slate-900/40 flex justify-end px-16">
              <button onClick={() => setShowModal(false)} className="px-14 py-5 bg-white text-black font-black rounded-2xl hover:bg-slate-200 transition-all uppercase text-xs tracking-[0.2em] shadow-xl">關閉並返回管理介面</button>
            </div>
          </div>
        </div>
      )}
