...
/** * 【全真實兩層穿透引擎邏輯】
 * 1. 目標探針 (Q)：抓取 Q 客戶地址的真實流入/流出流水（採樣 10 筆）。
 * 2. 真實客戶採集 (Level 1)：掃描您註冊店家的「流向 (OUT)」對象，這就是該店的【真實顧客地址】。
 * 3. 樣本流水穿透 (Level 2)：抓取這些【真實顧客】在鏈上的歷史流水。
 * 4. 絕對白名單排除：若共同點是您名單中的「公司錢包」，則不算關聯，直接剔除不顯示。
 * 5. 命中判定：
 * - 第一層直接命中：Q 地址 === 某分店的真實客戶地址。
...
function App() {
  ...
  const runRealDeepAnalysis = async () => {
    ...
    try {
        ...
        for (const clientAddr of clients) {
          ...
          if (cAddrLower === targetQ) {
            matches.push({
              ...
              description: `目標地址 Q 曾直接從「${merchant.name}」分店領取資金或有業務往來，兩者地址完全一致。`
            });
            continue; 
          }

          // 【比對模式 2】：穿透 Level 2 流水比對
          const cRes = await fetch(`https://api.trongrid.io/v1/accounts/${clientAddr}/transactions/trc20?limit=5`, {
            headers: { 'TRON-PRO-API-KEY': TRONGRID_API_KEY }
          });
          ...
          for (const cTx of cLedger) {
            ...
            if (cf === targetQ || ct === targetQ) {
              matches.push({
                ...
                description: `目標 Q 與「${merchant.name}」分店真實客戶 ${clientAddr} 在鏈上有過直接資金流向。`
              });
              break; 
            }
            ...
            if (qThirdPartyNodes.has(cf) || qThirdPartyNodes.has(ct)) {
              ...
              matches.push({
                ...
                description: `目標 Q 與「${merchant.name}」分店真實客戶 ${clientAddr} 均曾與第三方地址 ${commonNode} 有過資金往來。`
              });
              break;
            }
          }
      ...
      console.error(error);
      setAnalysisStep(0);
      alert("數據抓取異常，請確認 API Key 有效性或 TronGrid 服務狀態。");
    }
  };
            ...
            <section className="bg-slate-900/40 border border-slate-800 rounded-3xl p-8 backdrop-blur-md sticky top-28 shadow-2xl">
              ...
              <div className="space-y-6">
                <div className="bg-blue-500/5 border border-blue-500/10 p-4 rounded-2xl">
                    <p className="text-[10px] text-blue-400 font-bold mb-2 uppercase tracking-widest flex items-center gap-2"><Info size={14}/> 兩層穿透邏輯</p>
                    <p className="text-[11px] text-slate-500 italic leading-relaxed">
                        系統將即時掃描分店流向地址識別「真實客戶(L1)」，並進一步穿透其「真實流水(L2)」，排除公司白名單誤報。
                    </p>
                </div>
          ...
          {finalReport ? (
            <div className="bg-slate-900/60 border border-slate-800 rounded-[2.5rem] shadow-2xl animate-in pb-10">
              ...
              <div className="p-10 space-y-12">
                <section>
                  <h4 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-6 px-2 flex items-center gap-2"><Clock size={16} className="text-blue-500" /> 第一層：Q 真實鏈上交易流水</h4>
                ...
                <section>
                  <h4 className="text-[11px] font-bold text-red-500 uppercase tracking-widest mb-8 px-2 flex items-center gap-2"><AlertTriangle size={16} /> 兩層命中結果 (真實地址比對)</h4>
                  <div className="space-y-6">
                      ...
                      <div key={i} className="bg-slate-900/80 border border-slate-800 rounded-[2.5rem] p-8 flex flex-col md:flex-row gap-8 items-center group hover:border-red-500/30 transition-all shadow-xl">
                        ...
                        <div className="p-20 text-center bg-black/20 rounded-[2rem] border border-dashed border-slate-800 animate-in">
                            ...
                            <p className="text-sm font-bold text-slate-500 italic leading-relaxed">
                                經兩層主網穿透分析，未發現 Q 地址與註冊店鋪客戶在外部節點上有資金匯集跡象。
                            </p>
                        </div>
                    )}
                  </div>
            ...
            <div className="h-full min-h-[750px] flex flex-col items-center justify-center bg-slate-900/20 rounded-[4rem] border-2 border-dashed border-slate-800/50 text-slate-600 p-20 text-center shadow-inner">
               ...
               <p className="text-sm mt-4 text-slate-500 max-w-md leading-relaxed italic">請輸入 Q 地址發起掃描。系統將自動掃描分店流向並穿透真實客戶流水，排除公司錢包誤報。</p>
            </div>
          )}
        </div>
      </main>
