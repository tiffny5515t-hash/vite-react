...
// --- 實戰設定區 ---
// 您的 TronGrid API Key (請確保該 Key 有效)
const TRONGRID_API_KEY = "ab5d8c77-fee7-4fcc-a533-faa18a67f2c1"; 

/** * 【全真實兩層穿透審計引擎】
 * 1. 目標 Q 流水：從波場鏈真實抓取 Q 地址的最新 10 筆交易。
 * 2. 分店客戶識別 (L1)：掃描您錄入的公司錢包之 OUT 流向，定義為該店的【真實歷史顧客】。
 * 3. 深度穿透 (L2)：針對 L1 顧客再次發起 API 請求，獲取其真實流水。
 * 4. 智慧排除：若共同資金點為公司錢包，則視為正常業務白名單，不予顯示。
 * 5. 命中判定：Q 地址直接等於分店客戶，或兩者擁有共同的非公司第三方資金節點。
 */
...
function App() {
  ...
  const runRealDeepAnalysis = async () => {
    ...
    try {
      ...
      if (!qData.success) throw new Error("Q API Failed");
      ...
      // 2. 識別各分店真實客戶 (Level 1: 公司錢包 OUT 地址)
          ...
          for (const cTx of cLedger) {
            ...
            if (cf === targetQ || ct === targetQ) {
              matches.push({
                ...
                description: `目標 Q 與「${merchant.name}」分店真實客戶 ${clientAddr} 在鏈上有過直接資金流動紀錄。`
              });
              break; 
            }
            ...
            if (qNeighbors.has(cf) || qNeighbors.has(ct)) {
              ...
              matches.push({
                ...
                description: `目標 Q 與「${merchant.name}」分店真實客戶 ${clientAddr} 均與第三方地址 ${commonNode} 有過資金重疊（已排除公司錢包）。`
              });
              break;
            }
          }
      ...
      setAnalysisStep(0);
      alert("分析異常：可能達到 API 頻率限制，請稍後再試。");
    }
  };
          ...
          {activeTab === 'manager' ? (
            <section className="bg-slate-900/40 border border-slate-800 rounded-3xl p-6 backdrop-blur-md sticky top-28 shadow-xl shadow-blue-900/5">
              ...
              <div className="space-y-4">
                <input type="text" value={newMerchantName} onChange={e => setNewMerchantName(e.target.value)} placeholder="店面名稱 (如：BB分店)" className="w-full bg-black/50 border border-slate-800 rounded-xl px-4 py-4 text-sm focus:ring-1 focus:ring-blue-500 outline-none transition-all" />
            ...
            <section className="bg-slate-900/40 border border-slate-800 rounded-3xl p-8 backdrop-blur-md sticky top-28 shadow-2xl">
              ...
              <div className="space-y-6">
                <div className="bg-blue-500/5 border border-blue-500/10 p-4 rounded-2xl text-[11px]">
                    ...
                    <p className="text-slate-500 italic">系統將自動掃描分店「流向地址 (OUT)」識別為 L1 真實客戶，並進一步穿透 L2 真實流水，排除公司錢包誤報。</p>
                </div>
          ...
          {finalReport ? (
            <div className="bg-slate-900/60 border border-slate-800 rounded-[2.5rem] shadow-2xl animate-in pb-10">
              ...
              <div className="p-10 space-y-12">
                ...
                <section>
                  <h4 className="text-[11px] font-bold text-red-500 uppercase tracking-widest mb-8 px-2 flex items-center gap-2"><AlertTriangle size={16} /> 真實兩層碰撞命中結果 (已排除公司錢包)</h4>
                  <div className="space-y-6">
                    {finalReport.matches.length > 0 ? finalReport.matches.map((m, i) => (
                      <div key={i} className="bg-slate-900/80 border border-slate-800 rounded-[2.5rem] p-8 flex flex-col md:flex-row gap-8 items-center group hover:border-red-500/30 transition-all shadow-xl">
                        ...
                        <div className="flex-1 overflow-hidden space-y-4 w-full">
                          ...
                          <div className="flex justify-end gap-3 mt-4"><button onClick={() => {setModalData(m); setShowModal(true);}} className="bg-emerald-600 text-black font-black px-8 py-3 rounded-xl text-[10px] uppercase tracking-widest hover:bg-emerald-500 transition-all shadow-lg shadow-emerald-500/10">查看詳細證據</button></div>
                        </div>
                      </div>
                    )) : (
                        <div className="p-20 text-center bg-black/20 rounded-[2rem] border border-dashed border-slate-800 animate-in">
                            ...
                            <p className="text-sm font-bold text-slate-500 italic leading-relaxed">
                                經兩層主網穿透分析，Q 地址與各分店歷史客戶在非公司節點上無資金匯集跡象。
                            </p>
                        </div>
                    )}
                  </div>
                ...
                <div className="pt-10 border-t border-slate-800 flex justify-center">
                  <button onClick={() => {setQAddress(''); setFinalReport(null); setAnalysisStep(0);}} className="flex items-center gap-3 text-xs font-bold text-slate-500 hover:text-white transition-all uppercase bg-slate-900/50 px-12 py-4 rounded-full border border-slate-800 shadow-xl"><Repeat size={16} /> 執行新任務</button>
                </div>
              </div>
            </div>
          ) : (
            <div className="h-full min-h-[750px] flex flex-col items-center justify-center bg-slate-900/20 rounded-[4rem] border-2 border-dashed border-slate-800/50 text-slate-600 p-20 text-center shadow-inner">
               ...
               <p className="text-sm mt-4 text-slate-500 max-w-md leading-relaxed italic">請在左側輸入 Q 地址。系統將自動掃描分店流向地址識別「真實客戶」並執行深度兩層穿透比對。</p>
            </div>
          )}
        </div>
      </main>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 animate-in">
          ...
          <div className="relative w-full max-w-7xl bg-[#0c0c0e] border border-emerald-500/30 rounded-[3rem] shadow-2xl flex flex-col max-h-full overflow-hidden">
            <div className="p-8 border-b border-slate-800 flex justify-between items-center bg-gradient-to-r from-emerald-950/20 to-transparent px-12">
              <div className="flex items-center gap-5"><div className="bg-emerald-500 p-3 rounded-2xl shadow-lg shadow-emerald-500/20"><Activity className="text-black" size={28} /></div><div><h4 className="text-2xl font-black text-white italic uppercase tracking-tighter leading-none">真實穿透鏈上證據</h4><p className="text-[10px] text-emerald-500 font-bold uppercase tracking-widest mt-1">Status: Traceability Confirmed</p></div></div>
            ...
            <div className="p-12 overflow-y-auto custom-scrollbar space-y-12 px-16">
              ...
              <div className="bg-emerald-500/5 p-12 rounded-[3rem] border border-emerald-500/20 shadow-inner px-16">
                ...
                <p className="text-[16px] text-slate-400 leading-relaxed italic max-w-5xl">
                    {modalData.description} 該結果經由兩層主網穿透驗證，已自動排除與公司錢包相關的所有流水。目前偵測到的關聯，是基於雙方在非公司控制的外部共同節點「${modalData.relatedAddr}」有顯著資金匯集跡象。
                </p>
              </div>
            </div>
            <div className="p-10 border-t border-slate-800 bg-slate-900/40 flex justify-end px-16"><button onClick={() => setShowModal(false)} className="px-14 py-5 bg-white text-black font-black rounded-2xl hover:bg-slate-200 transition-all uppercase text-xs tracking-widest shadow-xl shadow-blue-500/10">關閉</button></div>
          </div>
        </div>
      )}
    </div>
